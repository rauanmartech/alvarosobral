...
// Assets
import graffitiShape from '@/assets/elements/grafite-shape.webp';
    ...
    async function loadGraffiti() {
      try {
        const { data, error } = await supabase
          .from("portfolio_graffiti")
          .select("*")
          .order("created_at", { ascending: true });

        if (error) {
          console.error("Error loading graffiti:", error);
        ...
        console.error("Unexpected error loading graffiti:", err);
      }

      // Fallback
      const initial = Array.from({ length: 9 }, (_, i) => ({
        id: `g-${i}`,
        title: `Mural Graffiti #${i + 1}`,
        image: "",
        isStarred: i === 0
      }));
      setArtworks(initial);
          ...
          <div className="w-full px-6 py-12 lg:py-0 lg:h-full lg:min-h-[600px] flex items-center relative z-10">
            ...
            <div className="flex-1 z-10 flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="inline-flex items-center gap-3 px-6 py-2 bg-white border-2 border-black rounded-tr-2xl rounded-bl-2xl rounded-tl-sm rounded-br-sm mb-8 shadow-[4px_4px_0_0_black]">
                ...
                <span className="text-black text-[10px] font-black uppercase tracking-[0.2em] font-alpha">Portfólio de Graffiti</span>
              </div>
              ...
              <div className="relative w-full">
                ...
                <h1 className="relative z-10 text-6xl md:text-8xl font-black font-outfit mb-6 leading-[0.9] text-white uppercase">
                  ARTE NA <br />
                  <span className="text-[hsl(var(--accent-orange))]">RUA</span>
                </h1>
              </div>

              <p className="text-lg md:text-xl text-white/50 font-outfit mb-10 max-w-xl leading-relaxed">
                Trabalho com diferentes estilos visuais, adaptando cada projeto à identidade e ao perfil do cliente. Meu objetivo é criar ambientes e fachadas com mais personalidade e significado, valorizando o espaço e despertando novas percepções sobre ele.
              </p>
              ...
              <div className="flex flex-col md:flex-row items-center gap-6 mb-10 w-full lg:w-fit p-1">
                ...
                <div className="bg-white/5 border-2 border-dashed border-white/20 px-6 h-20 rounded-2xl flex flex-col justify-center min-w-[180px] w-full md:w-auto relative group overflow-hidden">
                  ...
                  <p className="text-white font-bold font-outfit">Spray & Pincel</p>
                </div>
                
                {/* Info Card 2 */}
                <div className="bg-white/5 border-2 border-dashed border-white/20 px-6 h-20 rounded-2xl flex flex-col justify-center min-w-[180px] w-full md:w-auto relative group overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-[hsl(var(--accent-orange))] opacity-50" />
                  <p className="text-white/30 text-[10px] font-black uppercase tracking-widest mb-1">Alcance</p>
                  <p className="text-white font-bold font-outfit">Projetos Nacionais</p>
                </div>
      ...
      <div className="container mx-auto px-6 pb-32">
        <div className="flex flex-col md:flex-row md:items-end justify-between items-center md:items-end text-center md:text-left mb-16 gap-6">
          <div className="flex flex-col items-center md:items-start">
            ...
            <p className="text-black/40 font-outfit mt-4 max-w-sm">Confira aqui algumas transformações que já fiz.</p>
          </div>
