...
// Assets
import digitalShape from '@/assets/elements/digital-shape.png';
...
const Ilustracoes = () => {
  ...
  useEffect(() => {
    async function loadIlustracoes() {
      try {
        const { data, error } = await supabase
          .from("portfolio_ilustracoes")
          .select("*")
          .order("created_at", { ascending: true });

        if (error) {
          console.error("Error loading ilustracoes:", error);
        ...
        console.error("Unexpected error loading ilustracoes:", err);
      }

      // Fallback
      const initial = Array.from({ length: 9 }, (_, i) => ({
        id: `d-${i}`,
        title: `Ilustração Digital #${i + 1}`,
        image: "",
        isStarred: i === 0
      }));
      setArtworks(initial);
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            ...
            <div className="flex-1 z-10 flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="inline-flex items-center gap-3 px-6 py-2 bg-white border-2 border-black rounded-tr-2xl rounded-bl-2xl rounded-tl-sm rounded-br-sm mb-8 shadow-[4px_4px_0_0_black] -rotate-2">
                ...
                <span className="text-black text-[10px] font-black uppercase tracking-[0.2em] font-alpha">Ilustração Digital</span>
              </div>
              ...
              <p className="text-lg md:text-xl text-white/50 font-outfit mb-10 max-w-xl leading-relaxed">
                No ambiente digital, a criatividade não tem limites. Explore minhas ilustrações autorais, onde o traço manual encontra a versatilidade tecnológica para criar peças únicas.
              </p>
            ...
            <div className="relative w-full lg:absolute lg:top-0 lg:right-0 lg:h-full lg:w-1/2 flex justify-center items-center min-h-[350px] lg:min-h-0 z-0">
              ...
              <img 
                src={digitalShape} 
                alt="Digital Shape" 
                className="relative z-10 w-full max-w-[260px] md:max-w-none h-auto object-contain object-center lg:absolute lg:top-1/2 lg:left-1/2 lg:-translate-x-1/2 lg:-translate-y-1/2 lg:w-full lg:h-full lg:scale-[1.15]"
              />
              ...
              <img 
                src={blobFace} 
                alt="" 
                className="absolute z-30 pointer-events-none select-none w-[50px] md:w-[80px] top-[10%] left-[8%] lg:left-auto lg:right-[85%]" 
                style={{ animation: 'floatA 5s ease-in-out infinite' }} 
              />
              <img 
                src={blobTattoo} 
                alt="" 
                className="absolute z-30 pointer-events-none select-none w-[45px] md:w-[65px] bottom-[15%] right-[8%] lg:right-[5%]" 
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between items-center md:items-end text-center md:text-left mb-16 gap-6">
          <div className="flex flex-col items-center md:items-start">
            <h2 className="text-5xl font-black font-syne uppercase leading-none">
              Projetos <br />
              <span className="text-[hsl(var(--accent-orange))]">Digitais</span>
            </h2>
            <p className="text-black/40 font-outfit mt-4 max-w-sm">Uma coleção de trabalhos desenvolvidos inteiramente em ambiente digital, explorando novos estilos e narrativas visuais.</p>
          </div>
